...
#This version adapts Redferne's batch script edit to keep the Aspect Ratio of the original file and probably won't be updated unless it's something major
...
def vars():
	...
	if screen == 'Top' or screen == 'top':
		size = "\"\'if(gt(a,5/3),400,-2)\':\'if(gt(a,5/3),-2,240)\'\",pad=400:240:\'if(gt(a,5/3),0,trunc((400-240*a)/2))\':\'if(gt(a,5/3),trunc((240-400/a)/2),0)\':black"
		rename = 'anim'
		s = '-t'
	else:
		size = "\"\'if(gt(a,4/3),320,-2)\':\'if(gt(a,4/3),-2,240)\'\",pad=320:240:\'if(gt(a,4/3),0,trunc((320-240*a)/2))\':\'if(gt(a,4/3),trunc((240-320/a)/2),0)\':black"
		rename = 'bottom_anim'
		s = '-b'
	return
...
def convert():
	#Conversion steps
		...
		if sys.platform == 'win32':
			subprocess.call("ffmpeg -y -i %s -vf fps=%i,scale=%s,transpose=1 -pix_fmt bgr24 output.rgb" % (source, fps, size))
		else:
			subprocess.call("ffmpeg -y -i %s -vf fps=%i,scale=%s,transpose=1 -pix_fmt bgr24 output.rgb" % (source, fps, size), shell=True)
